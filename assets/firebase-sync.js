(function(){
  ...
  try{
    ...
    const calls={
      ...
      uploadBookingReceipt:callable('uploadBookingReceipt'),
      reviewBookingReceipt:callable('reviewBookingReceipt'),
    ...
    window.MFCloud={
      ...
      uploadBookingReceipt:async(bookingCode,file)=>{
        if(!calls.uploadBookingReceipt)throw new Error('Receipt upload function is unavailable');
        const base64=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result||'').split(',')[1]||'');reader.onerror=()=>reject(reader.error||new Error('File read failed'));reader.readAsDataURL(file);});
        return calls.uploadBookingReceipt({bookingCode:normalizeCode(bookingCode),fileName:file.name,contentType:file.type,base64});
      },
      reviewBookingReceipt:(code,approved,notes='')=>calls.reviewBookingReceipt({code:normalizeCode(code),approved,notes}),
      ...
      registerTeacherPushToken:async()=>{
        ...
        if(!registration.active){
          registration=await Promise.race([
            ...
            new Promise((_,reject)=>setTimeout(()=>reject(new Error('SERVICE_WORKER_ACTIVATION_TIMEOUT')),12000))
          ]);
        }
