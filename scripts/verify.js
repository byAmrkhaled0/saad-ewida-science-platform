...
for (const feature of [
  ...
  if (!mobileDesignSource.includes(feature)) fail(`V64 mobile-first design feature is missing: ${feature}`);
}
const desktopUnsafePrefix = mobileDesignSource.slice(0, mobileDesignSource.indexOf('@media (max-width: 760px)'));
if (/\.(?:hero|admin-sidebar|admin-main|admin-content|hero-grid)\s*\{/.test(desktopUnsafePrefix)) {
  fail('V64 stylesheet contains unscoped desktop layout overrides');
}
for (const htmlFile of htmlFiles) {
  if (!read(htmlFile).includes('assets/v64-mobile.css')) fail(`V64 stylesheet is missing from ${htmlFile}`);
}
const visibleSourceFiles = [
  ...htmlFiles,
  'assets/app.js', 'assets/admin.js', 'assets/online.js', 'assets/v53-upgrades.js',
  'assets/v56-fixes.js', 'functions/index.js', 'site.webmanifest', 'teacher.webmanifest'
];
for (const relative of visibleSourceFiles) {
  if (/أستاذ|استاذ|الأستاذ|الاستاذ/.test(read(relative))) fail(`Legacy teacher title remains in ${relative}`);
}
if (!failures.some(x => x.includes('V64') || x.includes('Legacy teacher title'))) ok('V64 mobile-first design and Mister naming checks passed');

const onlineHtml = read('online.html');
const storageCompatIndex = onlineHtml.indexOf('firebase-storage-compat.js');
const firebaseSyncIndex = onlineHtml.indexOf('assets/firebase-sync.js');
if (storageCompatIndex < 0 || firebaseSyncIndex < 0 || storageCompatIndex > firebaseSyncIndex) {
  fail('Online page must load Firebase Storage compat before firebase-sync.js');
}
if (!failures.some(x => x.includes('Firebase Storage compat'))) ok('Online Firebase runtime dependency order passed');

const contactSource = read('assets/app.js');
if (!contactSource.includes(':not(.engineer-only-name):not(.engineer-only-whatsapp)')) {
  fail('Teacher WhatsApp selector can overwrite engineer contact links');
}
if (!contactSource.includes("whatsappLink(ENGINEER_WHATSAPP")) {
  fail('Engineer WhatsApp links are not restored to the engineer number');
}
if (!failures.some(x => x.includes('WhatsApp'))) ok('Teacher and engineer WhatsApp links are isolated');

const vercelConfig = JSON.parse(read('vercel.json'));
const questionsRedirect = (vercelConfig.redirects || []).some(item =>
  item.source === '/questions.html'
    && item.destination === '/materials.html'
    && item.permanent === true
);
if (!questionsRedirect) fail('Permanent questions.html to materials.html redirect is missing');
if (read('scripts/build.js').includes("'questions.html': [")) fail('Redirect-only questions page must not be included in generated SEO sitemap pages');
if (!failures.some(x => x.includes('questions.html'))) ok('Questions redirect and sitemap configuration passed');

if (!onlineHtml.includes('aria-label="تصفية حسب الصف"') || !onlineHtml.includes('aria-label="تصفية حسب الترم"') || !onlineHtml.includes('aria-label="تصفية حسب الوحدة"')) {
  fail('Online filter accessible names are missing');
}
const reviewsHtml = read('reviews.html');
if (!reviewsHtml.includes('id="reviewName"') || !reviewsHtml.includes('for="reviewName"') || !reviewsHtml.includes('id="reviewText"') || !reviewsHtml.includes('for="reviewText"')) {
  fail('Review form labels are not programmatically associated');
}
if (!reviewsHtml.includes('id="themeToggle" aria-label="تغيير الوضع"')) fail('Reviews theme toggle accessible name is missing');
if (!read('index.html').includes('aria-describedby="bookingGroupHint"')) fail('Booking group accessible description is missing');
if (!failures.some(x => x.includes('accessible') || x.includes('programmatically associated'))) ok('Public form accessibility checks passed');

if (!read('assets/v61-ui.js').includes("firstSection?.contains(el)")) {
  fail('Above-the-fold content can still be hidden by reveal animation');
} else {
  ok('Above-the-fold content remains visible during first paint');
}
...
if (!adminSourceCode.includes("if(/not-found|الحجز غير موجود|تم التعامل معه/i.test(raw))")) fail('Accepted booking queue cleanup recovery is missing');
if (!adminSourceCode.includes('importCenterSchedulesV683') || !adminSourceCode.includes('scheduleGradeFilter') || !adminSourceCode.includes('scheduleStatusFilter') || !adminSourceCode.includes('ملاحظات الموعد')) fail('Schedule presets, filters, or notes are incomplete');
if (!appSourceCode.includes('item.notes].filter(Boolean)') || !adminSourceCode.includes("id:'saad-center-s1-g1'") || adminSourceCode.includes("id:'saad-center-s2")) fail('Public schedule notes or agreed center presets are incomplete');
...
if (read('index.html').includes('id="bookingGroup" name="group" aria-describedby="bookingGroupHint" required')) fail('Booking must not require a published schedule');
if (!read('assets/app.js').includes('وسيتم تحديد موعد') || !read('functions/index.js').includes('schedulePending: !selectedScheduleId')) fail('Schedule-optional booking flow is incomplete');
...
if (!adminSourceCode.includes('firebase-messaging-compat.js') || !adminSourceCode.includes('ensureFirebaseMessaging') || !sw.includes('/assets/vendor/firebase-messaging-worker-10.12.5.min.js') || !read('assets/vendor/firebase-messaging-worker-10.12.5.min.js').includes('onBackgroundMessage')) fail('Teacher background push notification wiring is incomplete');
...
if (packageInfo.version !== '69.2.2' || !read('assets/app.js').includes("MF_ASSET_VERSION = '69.2.2'") || !read('service-worker.js').includes('mf-science-v6922-admin-performance') || [...functionsSource.matchAll(/'platform-release': '69-2-0'/g)].length !== 2) fail('V69.2.2 frontend and V69.2.0 backend release identifiers are invalid');
...
if (!adminSourceCode.includes('adminRecordSections') || !adminSourceCode.includes('ensureAdminRecords') || !read('scripts/build.js').includes("adminCssParts = ['site.css','v61.css','v64-mobile.css']")) fail('V69.2.2 lazy admin records or focused admin stylesheet is incomplete');
...
for(const feature of ['adminGlobalSearch','admin-quick-add','admin-attention-list','quickAdminCreate','admin-nav-group','adminSaveState']){
  if(!staffAdminSource.includes(feature)&&!read('assets/site.css').includes(feature))fail(`Simplified admin feature is missing: ${feature}`);
}
if(!read('assets/app.js').includes('header-booking-btn')||read('assets/app.js').includes('header-call-btn'))fail('Public header must show one booking action while contact setup owns the call action');
if(!failures.some(x=>x.includes('Simplified admin')||x.includes('Public header')))ok('Simplified admin navigation, quick actions, search, and header actions passed');
